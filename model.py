...
# from model_hyper_bak import HyperGCN
...
class Model(nn.Module):
    ...
    def __init__(self, base_model, D_m, D_g, D_p, D_e, D_h, D_a, graph_hidden_size, n_speakers, max_seq_len, window_past, window_future, 
                 ...
                 use_speaker=True, use_modal=False, norm='LN2', num_L = 3, num_K = 4, windows=5 , step=6):
        ...
        if self.graph_type=='hyper':
            self.graph_model = HyperGCN(a_dim=D_g, v_dim=D_g, l_dim=D_g, n_dim=D_g, nlayers=64, nhidden=graph_hidden_size,
                                        nclass=n_classes, dropout=self.dropout, lamda=0.5, alpha=0.1, variant=True,
                                        return_feature=self.return_feature, use_residue=self.use_residue, n_speakers=n_speakers,
                                        modals=self.modals, use_speaker=self.use_speaker, use_modal=self.use_modal, num_L=num_L, num_K=num_K,windows=windows,step=step)
    ...
    def forward(self, U, qmask, umask, seq_lengths, U_a=None, U_v=None, epoch=None):
        ...
        if self.graph_type=='GCN3' or self.graph_type=='DeepGCN':
            ...
            emotions_feat = self.dropout_(emotions_feat)
            emotions_feat = nn.ReLU()(emotions_feat)
            temp = self.smax_fc(emotions_feat)
            log_prob = F.log_softmax(temp, 1)
        else:
            print("There are no such kind of graph")        
